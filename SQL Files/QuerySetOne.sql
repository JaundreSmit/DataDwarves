...
SELECT 
    ...
    SUM(CASE WHEN C.available = 'f' THEN 1 ELSE 0 END) / COUNT(C.date) * 100 AS occupancy_rate
FROM Listings L
JOIN Calendar C ON L.listing_id = C.listing_id
GROUP BY L.neighbourhood_group
...
SELECT
    ...
    END AS season,
    ROUND(AVG(C.price), 2) AS average_rental_income,
    SUM(CASE WHEN C.available = 'f' THEN 1 ELSE 0 END) / COUNT(C.date) * 100 AS occupancy_rate
FROM Calendar C
...
WHERE
    ...
    OR R.comments LIKE '%amazing%'
    OR R.comments LIKE '%comfortable%'
    OR R.comments LIKE '%wonderful%'
    OR R.comments LIKE '%clean%'
    OR R.comments LIKE '%perfect%'
